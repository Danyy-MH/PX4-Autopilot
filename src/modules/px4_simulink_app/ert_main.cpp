...
void *baseRateTask(void *arg)
{
  runModel = (modelDeployment_M->getErrorStatus() == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    modelDeployment_step();

    // Get model outputs here
    stopRequested = !((modelDeployment_M->getErrorStatus() == (NULL)));
  }
...
{
  UNUSED(sig);
  modelDeployment_M->setErrorStatus("stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  modelDeployment_M->setErrorStatus(0);

  // Initialize model
  modelDeployment_initialize();
